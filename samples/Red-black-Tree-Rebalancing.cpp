...
struct Node {
  Node() = default;
  Node(Color color_, std::shared_ptr<Node> const& lhs_, T value_, std::shared_ptr<Node> const& rhs_)
    : color{color_}
    , lhs{lhs_}
    , value{value_}
    , rhs{rhs_}
...
{
    return lhs.color == rhs.color && lhs.lhs == rhs.lhs && lhs.value == rhs.value && lhs.rhs == rhs.rhs;
}
...
int main()
{
    auto x = std::make_shared<Node<int>>(Color::Red, std::shared_ptr<Node<int>>{}, 1, std::shared_ptr<Node<int>>{});
    auto y = std::make_shared<Node<int>>(Color::Red, std::shared_ptr<Node<int>>{}, 2, x);
    auto z = std::make_shared<Node<int>>(Color::Black, std::shared_ptr<Node<int>>{}, 3, y);
